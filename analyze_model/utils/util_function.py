...
def merge_and_slice_features(featin, is_gt: bool, feat_type: str):
    ...
    if feat_type.startswith("feat"):
        ...
        newfeat = (slice_feature(featin, composition))
...
# TODO delete
def merge_and_slice_features1(features, is_gt):
    """
    ...
    for raw_feat in features["feat_lane"]:
        merged_feat = merge_dim_hwa(raw_feat)
        lane_compos = uc.get_lane_channel_composition(is_gt)
        sliced_features["feat_lane"].append(slice_feature(merged_feat, lane_compos))
    sliced_features["feat_lane"] = scale_align_featmap(sliced_features["feat_lane"])
    ...
    if "lanes" in features["inst"]:
        ...
        b, n, c = features["inst"]["lanes"].shape
        five_points = tf.reshape(features["inst"]["lanes"], (b, n, c))
        sliced_features["inst"]["lanes"] = slice_feature(five_points, lane_compos)
...
def scale_align_featmap(features):
    slice_keys = list(features.keys())
    aline_feat = dict()
    for key in slice_keys:
        aline_feat[key] = [features[key]]
    return aline_feat
