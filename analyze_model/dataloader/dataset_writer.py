...
def drive_manager_factory(dataset_name, split, srcpath):
    if dataset_name == "analyze":
        from dataloader.readers.analyze_reader import AnalyzeDriveManager
        return AnalyzeDriveManager(srcpath, split)
    else:
...
def drive_reader_factory(dataset_name, dataset_cfg, split, drive_path):
    if dataset_name == "analyze":
        from dataloader.readers.analyze_reader import AnalyzeReader
        return AnalyzeReader(drive_path, split, dataset_cfg)
    else:
...
class TfrecordMaker:
    ...
    def write_drive(self, drive_path):
        ...
        example_maker = ExampleMaker(data_reader, self.dataset_cfg, self.tfr_drive_path)
    ...
    def verify_example(self, drive_example, example):
        if (not example) or ("data" not in example):
    ...
    def write_tfrecord_config(self, example):
        assert ('data' in example) and (example['data'] is not None)
